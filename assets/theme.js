            ...
            if (state.mostRecentlyFocusedNode) {
                if (getTabIndex(state.mostRecentlyFocusedNode) > 0) {
                    ...
                    if (tabbableNodes.length > 0) {
                        var mruTabIdx = tabbableNodes.findIndex(function (
                            node
                        ) {
...
var LoaderButtonMixin = {
    ...
    _startTransition() {
        ...
        this._startTransitionPromise = Promise.all([
            new Promise(
                (resolve) => (textAnimation.onfinish = () => resolve())
            ),
            new Promise(
                (resolve) => (spinnerAnimation.onfinish = () => resolve())
            ),
        ]);
    },
...
var ScrollSpy = class extends HTMLElement {
    ...
    syncPath() {
        ...
        if (someElsAreVisible() && pathStart < pathEnd) {
            if (
                pathStart !== this.lastPathStart ||
                pathEnd !== this.lastPathEnd
            ) {
        ...
        requestAnimationFrame(() => {
            for (const position of ["top", "bottom", "left", "right"]) {
                targetElement.style.setProperty(
                    `--${position}`,
                    `${
                        scroll[position] > maxSize ? maxSize : scroll[position]
                    }px`
                );
            }
        });
...
var NativeCarousel = class extends CustomHTMLElement {
    ...
    _adjustNavigationForElement(selectedElement) {
        this.items.forEach(
            (item) => (item.selected = selectedElement === item)
        );
            ...
            requestAnimationFrame(() => {
                ...
                this.innerHTML = Object.values(bounds)
                    .map(
                        (item) =>
                            `<span ${
                                this.hasAttribute("reveal") && !force
                                    ? "reveal"
                                    : ""
                            } ${
    ...
    connectedCallback() {
        ...
        if (this.hasAttribute("data-default")) {
            for (let i = 0; i !== this.options.length; ++i) {
                if (
                    this.options[i].text === this.getAttribute("data-default")
                ) {
...
var PriceRange = class extends HTMLElement {
    connectedCallback() {
        ...
        this.rangeLowerBound.addEventListener("input", (event) => {
            ...
            event.target.value = Math.min(
                parseInt(event.target.value),
                parseInt(this.textInputHigherBound.value || event.target.max) -
                    1
            );
...
var FlickityCarousel = class extends CustomHTMLElement {
    ...
    async _onStaticClick(event, pointer, cellElement) {
        const flickityInstance = await this.flickityInstance,
            isVideoOrModelType =
                flickityInstance.selectedElement.hasAttribute(
                    "data-media-type"
                ) &&
                ["video", "external_video", "model"].includes(
                    flickityInstance.selectedElement.getAttribute(
                        "data-media-type"
                    )
                );
        if (
    ...
    async _preloadNextImage() {
        ...
        if (flickityInstance.selectedElement.nextElementSibling) {
            flickityInstance.selectedElement.nextElementSibling
                .querySelector("img")
...
var FlickityControls = class extends CustomHTMLElement {
    ...
    async _onSlideChanged(animate = true) {
        ...
        activeItems.forEach((activeItem) => {
            ...
            requestAnimationFrame(() => {
                if (
                    activeItem.offsetParent &&
                    activeItem.offsetParent !== this
                ) {
                    const windowHalfHeight =
                            activeItem.offsetParent.clientHeight / 2,
                        windowHalfWidth =
                            activeItem.offsetParent.clientWidth / 2;
...
var ComboBox = class extends OpenableElement {
    ...
    _onValueClicked(event, target) {
        ...
        if (this.nativeControl.value !== target.value) {
            ...
            this.nativeControl.dispatchEvent(
                new Event("change", { bubbles: true })
            );
        }
        this.open = false;
    ...
    _onValueChanged(event, target) {
        ...
        if (nativeControl.tagName === "SELECT") {
            Array.from(this.nativeControl.options).forEach((option) =>
            ...
            responses.map(async (response) => {
                ...
                const categoryResultDiv = div.querySelector(
                    ...
                    tabNavItem = div.querySelector(
                        "#search-tabs-nav .tabs-nav__item"
                    );
    ...
    async connectedCallback() {
        ...
        this.addEventListener("split-lines:re-split", (event) => {
            Array.from(event.target.children).forEach(
                (line) =>
                    (line.style.visibility = this.selected
                        ? "visible"
                        : "hidden")
            );
        });
    ...
    async transitionToLeave(transitionType, shouldAnimate = true) {
        ...
        let animation = null,
            textElements = await resolveAsyncIterator(
                this.querySelectorAll(
                    "split-lines, .button-group, .button-wrapper"
                )
            ),
    ...
    async select(index, shouldTransition = true, reverseDirection = false) {
        ...
        if (previousItem && previousItem !== newItem) {
            if (this.transitionType !== "reveal") {
                previousItem.transitionToLeave(
                    this.transitionType,
                    shouldTransition
                );
            } else {
...
var ImageWithTextItem = class extends HTMLElement {
    ...
    async transitionToEnter(shouldAnimate = true) {
        this.removeAttribute("hidden");
        const textWrapper = this.querySelector(
        ...
        const activeImage = this.imageItems.find(
            ...
            nextImage =
                this.imageItems.find(
                    (item) =>
                        item.id ===
                        this.items[index].getAttribute("attached-image")
                ) || activeImage;
...
var ProductList = class extends CustomHTMLElement {
                        ...
                        return new CustomKeyframeEffect(
                            ...
                            },
                            {
                                ...
                                delay: MediaFeatures.prefersReducedMotion()
                                    ? 0
                                    : 100 * index -
                                      Math.min(3 * index * index, 100 * index),
                                easing: "ease",
                            }
                        );
                    })
    ...
    next(event) {
        ...
        this.productListInner.scrollBy({
            left:
                (this.productListInner.clientWidth + columnGap) * directionFlip,
            behavior: "smooth",
        });
    }
    attributeChangedCallback(name) {
        ...
        switch (name) {
            case "hidden":
                if (!this.hidden) {
                    ...
                    this.querySelector(".prev-next-button--prev")?.setAttribute(
                        "disabled",
                        ""
                    );
                    this.querySelector(
    ...
    _updateProgressBar() {
        const stickyHeaderOffset = getStickyHeaderOffset(),
            ...
            progress = Math.max(
                -1 *
                    (difference /
                        (articleMainPartBoundingBox.height +
                            marginCompensation) -
                        1),
                0
            );
...
var MultiColumn = class extends CustomHTMLElement {
    ...
    next(event) {
        ...
        this.multiColumnInner.scrollBy({
            left:
                (this.multiColumnInner.clientWidth + columnGap) * directionFlip,
            behavior: "smooth",
        });
    }
};
...
var GalleryList = class extends HTMLElement {
    ...
    _updateProgressBar(event) {
        this.scrollBarElement?.style.setProperty(
            "--transform",
                ...
                contentElement.style.opacity = Math.max(
                    1 -
                        speedFactor *
                            (1 -
                                Math.min(
                                    boundingRectBottom / boundingRectHeight,
                                    1
                                )),
                    0
                ).toString();
...
var PredictiveSearchDrawer = class extends DrawerContent {
        ...
        if (target.value === "") {
            ...
            try {
                ...
                if (this._supportPredictiveApi()) {
                    searchResults = await this._doPredictiveSearch(
                        target.value
                    );
                } else {
    ...
    async _doPredictiveSearch(term) {
        const response = await fetch(
            `${
                ...
                term
            )}&resources[limit]=10&resources[limit_scope]=each&section_id=predictive-search`,
            {
        ...
        return {
            hasResults:
                div.querySelector(".predictive-search__results-categories") !==
                null,
    ...
    async _doLiquidSearch(term) {
        const response = await fetch(
            `${window.themeVariables.routes.searchUrl}?q=${encodeURIComponent(
                term
            )}&resources[limit]=50&section_id=predictive-search-compatibility`,
            {
                signal: this.abortController.signal,
            }
        );
    ...
    connectedCallback() {
        ...
        if (this.listItemElements.length > 1) {
            ...
            if (Shopify.designMode) {
                this.addEventListener("shopify:block:select", (event) => {
                    this.select(
                        [...event.target.parentNode.children].indexOf(
                            event.target
                        ),
                        !event.detail.load
                    );
    ...
    openDropdown(parentElement) {
        ...
        let openingTimeout = setTimeout(() => {
            ...
            if (
                ...
                items.forEach((item) => {
                    item.getAnimations().forEach((animation2) =>
                        animation2.cancel()
                    );
...
var GiftCardRecipient = class extends HTMLElement {
    connectedCallback() {
        ...
        this.recipientCheckbox?.addEventListener(
            "change",
            this._synchronizeProperties.bind(this)
        );
        this.offsetProperty = this.querySelector(
            '[name="properties[__shopify_offset]"]'
        );
        if (this.offsetProperty) {
            this.offsetProperty.value = /* @__PURE__ */ new Date()
                .getTimezoneOffset()
                .toString();
        }
                ...
                (element) => {
                    ...
                    if (matchingBlock) {
                        if (blockType === "buy-buttons") {
                            element
                                .querySelector("product-payment-container")
                                .replaceWith(
                            ...
                            const existingQuantity = element.querySelector(
...
var PhotoSwipeUi = class {
    ...
    init() {
        const prevNextButtons = this.pswpUi.querySelector(
            ...
            dotsNavWrapper = this.pswpUi.querySelector(
...
var ProductImageZoom = class extends OpenableElement {
    ...
    async _buildItems() {
        const activeImages = Array.from(
            ...
            product = await ProductLoader.load(
                this.getAttribute("product-handle")
            );
        return Promise.resolve(
            activeImages.map((item) => {
                const matchedMedia = product["media"].find(
                    ...
                    supportedSizes = getSupportedSizes(
                        matchedMedia,
                        [
                            200, 300, 400, 500, 600, 700, 800, 1e3, 1200, 1400,
                            1600, 1800, 2e3, 2200, 2400, 2600, 2800, 3e3,
                        ]
                    ),
...
var ProductForm = class extends HTMLFormElement {
    ...
    async _onSubmit(event) {
        const monster_active = !!document.querySelector(
            'script[src*="/cart_renderer."]'
        );
    ...
    async _onVariantChanged(event) {
        ...
        const currentlyFilteredIds = [
            ...new Set(
                ...
                ).map((item) => parseInt(item.getAttribute("data-media-id")))
            ),
        ];
        if (
            currentlyFilteredIds.some(
                (value) => !filteredMediaIds.includes(value)
            )
        ) {
            ...
            const flickityInstance = await this.mainCarousel.flickityInstance;
            flickityInstance.destroy();
            Array.from(this.querySelectorAll("[data-media-id]")).forEach(
                (item) => {
...
var QuickBuyDrawer = class extends DrawerContent {
    ...
    async _load() {
        ...
        const drawer = html
            .querySelector("#quick-buy-content")
            .content.querySelector("quick-buy-drawer");
...
var QuickBuyPopover = class extends PopoverContent {
    ...
    async _load() {
        ...
        const drawer = html
            .querySelector("#quick-buy-content")
            .content.querySelector("quick-buy-popover");
        this.replaceChildren(...drawer.children);
        triggerNonBubblingEvent(this, "openable-element:load:end");
        if (window.Shopify && window.Shopify.PaymentButton) {
            window.Shopify.PaymentButton.init();
    ...
    async connectedCallback() {
        ...
        __privateGet(this, _delegate).on(
            ...
            `input[data-option-position][form="${this.getAttribute(
                "form-id"
            )}"], select[data-option-position][form="${this.getAttribute(
                "form-id"
            )}"]`,
            __privateMethod(
                this,
...
getActiveOptionValues_fn = function () {
    return Array.from(__privateGet(this, _form).elements)
        .filter((item) =>
            item.matches(
                'input[data-option-position]:checked, input[data-option-position][type="hidden"]'
            )
        )
        .sort(
            (a, b) =>
...
getOptionValuesFromOption_fn = function (input) {
    const optionValues = [
        ...
        ...Array.from(__privateGet(this, _form).elements).filter((item) =>
            item.matches(
                `input[data-option-position]:not([name="${input.name}"]):checked, input[data-option-position]:not([name="${input.name}"])[type="hidden"]`
            )
        ),
    ]
        .sort(
...
onIntersection_fn = function (entries) {
    const prerenderOptions = () => {
        Array.from(__privateGet(this, _form).elements)
            .filter((item) =>
                item.matches(
                    'input[data-option-position]:not(:checked), input[data-option-position][type="hidden"]'
                )
            )
            .forEach((input) => {
                __privateMethod(
    ...
    if (!__privateGet(_VariantPicker, _preloadedHtml).has(hashKey)) {
        ...
        if (__privateGet(_VariantPicker, _preloadedHtml).size > 100) {
            __privateGet(_VariantPicker, _preloadedHtml).delete(
                Array.from(
                    __privateGet(_VariantPicker, _preloadedHtml).keys()
                )[0]
            );
        }
    }
            ...
            if (filtersTempDiv) {
                ...
                Array.from(
                    ...
                    const filtersTempDivToggle = filtersTempDiv.querySelector(
                        ...
                        isExpanded =
                            filterToggle.getAttribute("aria-expanded") ===
                            "true";
                    if (filtersTempDivToggle) {
                        ...
                        filtersTempDivToggle.nextElementSibling.style.overflow =
                            isExpanded ? "visible" : "";
                    }
                });
...
var _CartDrawerRecommendations = class _CartDrawerRecommendations extends HTMLElement {
    async connectedCallback() {
        if (!_CartDrawerRecommendations.recommendationsCache[this.productId]) {
            _CartDrawerRecommendations.recommendationsCache[this.productId] =
                fetch(
...
var ProductStickyForm = class extends HTMLElement {
    ...
    _setupVisibilityObservers() {
        const paymentContainerElement = document
                .getElementById("MainPaymentContainer")
                .closest("form"),
...
//Custom added by Zelta

document.addEventListener('DOMContentLoaded', function () {
  // Set the countdown time in seconds (e.g., 10 minutes = 600 seconds)
  const countdownTime = 10 * 60; // Change this value to your desired duration
  ...
  function updateTimer() {
    ...
    const checkoutButton = document.querySelector('.checkout-button.button.button--primary.button--full.ze-checkout'); // Targeting specific button by class
    ...
    if (remainingTime > 0) {
      ...
      // Disable the checkout button when the timer reaches 0
      if (checkoutButton) {
        checkoutButton.disabled = true;  // Disable the button
        checkoutButton.classList.add('disabled');  // Optionally add a 'disabled' class for styling
      }
    }
  }
 ...
 document.getElementById('add-to-cart-form').addEventListener('submit', function(event) {
    ...
    // Add the selected variant to the cart using AJAX
    fetch('/cart/add.js', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
    ...
    .then(data => {
      // Dispatch 'variant:added' event to notify that a variant has been added to the cart
      const variantAddedEvent = new CustomEvent('variant:added', {
        detail: {
          variant: data,
        }
      });
      document.dispatchEvent(variantAddedEvent);

      // Dispatch 'cart:updated' event to notify that the cart has been updated
      const cartUpdatedEvent = new CustomEvent('cart:updated', {
        detail: {
          cart: data,  // Updated cart data can be passed here
        }
      });
      document.dispatchEvent(cartUpdatedEvent);

      // Open the cart drawer after adding the item
      document.getElementById('mini-cart').open = true;  // Open the cart drawer using custom element method
    })
    .catch(error => {
  ...
  // Listen for 'cart:updated' event to refresh the cart or mini-cart UI if needed
  document.addEventListener('cart:updated', function(event) {
    var cart = event.detail.cart; // The updated cart details
    console.log(cart);  // Optionally log or use cart data to update UI
  });
